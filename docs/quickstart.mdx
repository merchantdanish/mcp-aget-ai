...
<Steps>
  ...
  <Step title="Install MCP servers">
    ...
    # Install filesystem server (requires Node.js)
...
<Warning>
  Replace `"your-openai-api-key-here"` with your actual OpenAI API key. You can get one from the [OpenAI platform](https://platform.openai.com/api-keys).
</Warning>
...
async def main():
    async with app.run() as mcp_agent_app:
        ...
        async with finder_agent:
            ...
            logger.info("Available tools:", data=[tool.name for tool in tools])
...
<AccordionGroup>
  ...
  <Accordion title="MCP Server Not Found">
    Ensure the MCP servers are installed:
    ```bash
    # Check if fetch server is installed
    uvx list | grep mcp-server-fetch
    
    # Check if filesystem server is installed
...
<CardGroup cols={2}>
  <Card title="Learn Core Concepts" icon="graduation-cap" href="/concepts/agents">
    Understand how agents, MCP servers, and LLMs work together
  </Card>
  <Card title="Explore Workflow Patterns" icon="diagram-project" href="/workflows/overview">
    Discover advanced patterns like parallel processing and routing
  </Card>
  <Card title="Try Different Providers" icon="shuffle" href="/providers/anthropic">
...
<Tip>
  Check out the [examples directory](https://github.com/lastmile-ai/mcp-agent/tree/main/examples) for 30+ working examples covering different use cases and patterns.
</Tip>