...
1. Separating traces by component (agent, LLM, MCP aggregator)
2. Analyzing component interactions and timeline
3. Extracting agent workflows and LLM interactions
4. Generating tree-like visualizations of spans
5. Creating detailed trace reports

Example usage:
    # Analyze a trace file
    detailed_analysis = get_detailed_trace_analysis("path/to/trace.jsonl")
    
    # Generate a report
    report = generate_trace_report(detailed_analysis)
    
    # Extract spans organized by feature
    spans = read_trace_file("path/to/trace.jsonl")
    feature_spans = extract_spans_by_feature(spans)
    
    # Create a feature tree visualization
...
def print_feature_spans_tree(feature_spans: Dict[str, List[Dict[str, Any]]]) -> str:
    ...
    for feature, spans in feature_spans.items():
        ...
        for i, span in enumerate(sorted_spans):
            ...
            # Skip printing all attributes to keep the tree more concise
