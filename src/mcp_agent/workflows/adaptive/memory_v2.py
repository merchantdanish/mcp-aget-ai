"""
Memory management for Adaptive Workflow V2

This module provides:
1. Storage abstraction for different backends
2. Pattern tracking for adaptive learning
3. Simple, extensible design that aligns with V2's philosophy
"""
...
from .models_v2 import ExecutionMemory, TaskType, SubagentResult
...
class AdaptiveMemory:
    ...
    async def learn_from_execution(self, memory: ExecutionMemory) -> None:
        ...
        for i, synthesis in enumerate(memory.research_history):
            ...
            relevant_results = [
                r for r in memory.subagent_results 
            ...
            if relevant_results:
                ...
                self.session_patterns["successful_approaches"][task_type].append(pattern)
    ...
    async def suggest_approach(self, objective: str, task_type: Optional[TaskType] = None) -> Optional[Dict[str, Any]]:
        """Suggest approach based on past successful patterns"""
        if not task_type:
            return None
        
        # Look for similar objectives in successful patterns
        task_patterns = self.session_patterns["successful_approaches"].get(task_type.value, [])
    ...
    def get_effective_tools(self, task_type: TaskType) -> List[str]:
        ...
        tool_scores = self.session_patterns["tool_effectiveness"].get(task_type.value, {})
...
class MemoryManager:
    ...
    def __init__(self, backend: Optional[MemoryBackend] = None, enable_learning: bool = True):
        """
