...
from typing import Callable, List, Optional, Type, Any, Tuple
...
class AdaptiveWorkflow(AugmentedLLM[MessageParamT, MessageT]):
    ...
    async def generate(
        ...
        with tracer.start_as_current_span(
            ...
            span.set_attribute(
                "workflow.time_budget_seconds", self.time_budget.total_seconds()
            )
    ...
    async def _analyze_task(
        ...
        if self.enable_learning:
            learned_complexity = self.learning_manager.estimate_complexity(
                objective, analysis.task_type
            )
            # Average the learned and analyzed complexity
            if learned_complexity != analysis.complexity:
                logger.info(
                    f"Adjusting complexity from {analysis.complexity} to {learned_complexity} based on learning"
                )
    ...
    async def _plan_strategy(
        ...
        if self.enable_learning:
            suggested_strategy = self.learning_manager.suggest_strategy(
        ...
        # Apply constraints
        strategy.parallelism_level = min(
            strategy.parallelism_level, 10 if self.enable_parallel else 1
        )
    ...
    async def _execute_iterations(
        ...
        while self._should_continue(start_time):
            ...
            if (
                ...
                and task_batch.can_parallelize
                and len(task_batch.tasks) > 1
            ):
            ...
            if evaluation.is_complete and evaluation.confidence > 0.8:
                logger.info(
                    f"Objective achieved with {evaluation.confidence:.1%} confidence"
                )
    ...
    async def _execute_parallel(
        ...
        async with asyncio.TaskGroup() as tg:
            for task, agent in agents:
                tg.create_task(
                    self._execute_single_task(task, agent, request_params, span)
                )
    ...
    async def _execute_sequential(
        ...
        for task in tasks:
            ...
            if not self._dependencies_met(task):
                logger.warning(
                    f"Skipping task {task.task_id} due to unmet dependencies"
                )
                task.status = "skipped"
                continue
        ...
        try:
            async with agent:
                ...
                if hasattr(task_span, "get_attribute"):
                    task.input_tokens = (
                        task_span.get_attribute(GEN_AI_USAGE_INPUT_TOKENS) or 0
                    )
                    task.output_tokens = (
                        task_span.get_attribute(GEN_AI_USAGE_OUTPUT_TOKENS) or 0
                    )
                    # Estimate cost (simplified - would need actual model pricing)
                    task.cost = (task.input_tokens / 1000) * 0.01 + (
                        task.output_tokens / 1000
                    ) * 0.03
    ...
    async def _synthesize_results(self, span: trace.Span) -> WorkflowResult:
        ...
        successful_tasks = [
            t for t in self._current_memory.completed_tasks if t.status == "completed"
        ]
        ...
        failed_tasks = [
            t for t in self._current_memory.completed_tasks if t.status == "failed"
        ]
        limitations = []
        if failed_tasks:
    ...
    async def _format_citations(
        self, result: WorkflowResult, span: trace.Span
    ) -> WorkflowResult:
    ...
    def _check_resource_limits(self, task_batch: TaskBatch, span: trace.Span) -> bool:
        ...
        estimated_subagents = self._current_memory.total_subagents_created + len(
            task_batch.tasks
        )
        estimated_cost = self._current_memory.total_cost + (
            len(task_batch.tasks) * 0.5
        )  # Rough estimate

        if estimated_subagents > self.max_subagents:
            span.add_event(
                "resource_limit_subagents",
                {
    ...
    def _dependencies_met(self, task: SubagentTask) -> bool:
        ...
        completed_ids = {
            t.task_id
            for t in self._current_memory.completed_tasks
    ...
    def _get_relevant_context(self, objective: str) -> str:
        ...
        if relevant_findings:
            return "Related findings:\n" + "\n".join(
                f"- {f}" for f in relevant_findings[:5]
            )
        else:
