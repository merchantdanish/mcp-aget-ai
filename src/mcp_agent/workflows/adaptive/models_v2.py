...
class ResearchAspect(BaseModel):
    ...
    required_servers: List[str] = Field(
        ...
        description="MCP servers needed for this research"
    )
...
class SynthesisDecision(BaseModel):
    ...
    confidence: float = Field(
        ge=0.0, le=1.0,
        description="Confidence that the objective is complete"
    )
    reasoning: str = Field(
    ...
    new_aspects: Optional[List[ResearchAspect]] = Field(
        default=None,
        description="New aspects to research if not complete"
    )
...
class ExecutionMemory(BaseModel):
    ...
    research_history: List[Any] = Field(
        default_factory=list,
        description="Synthesis messages from each iteration"
    )
    subagent_results: List[SubagentResult] = Field(
        default_factory=list,
        description="All subagent results"
    )
