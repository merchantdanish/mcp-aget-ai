...
class LearningManager:
    ...
    def __init__(self):
        ...
        self.patterns = {
            'task_strategies': defaultdict(lambda: {
                'successes': 0,
                'failures': 0,
                'avg_time': 0.0,
                'avg_cost': 0.0,
    ...
    def record_execution(self, memory: WorkflowMemory, success: bool = True) -> None:
        ...
        self.patterns['timing_data'][task_key] = self.patterns['timing_data'][task_key][-100:]
        ...
        stats['avg_cost'] = stats['avg_cost'] * (1 - weight) + memory.total_cost * weight
        stats['avg_iterations'] = stats['avg_iterations'] * (1 - weight) + memory.iterations * weight
        ...
        for task in memory.completed_tasks:
            if task.agent_spec.server_names:
                for server in task.agent_spec.server_names:
                    ...
                    current = self.patterns['tool_effectiveness'][memory.task_type][server]
                    self.patterns['tool_effectiveness'][memory.task_type][server] = (
                        current * 0.9 + effectiveness * 0.1  # Exponential moving average
                    )
    
    def suggest_strategy(self, task_type: TaskType, complexity: TaskComplexity) -> Optional[WorkflowStrategy]:
        ...
        for strategy_key, stats in self.patterns['task_strategies'].items():
            if strategy_key.startswith(str(task_type)):
                ...
                if total >= 3:  # Need at least 3 executions
                    ...
                    time_factor = 1.0 / (1.0 + stats['avg_time'] / 1800)  # 30 min baseline
    ...
    def estimate_complexity(self, objective: str, task_type: TaskType) -> TaskComplexity:
        """Estimate task complexity based on objective and type"""
        objective_lower = objective.lower()
        
        # Task type specific heuristics
        if task_type == TaskType.RESEARCH:
            if any(kw in objective_lower for kw in ['comprehensive', 'all', 'complete', 'detailed analysis']):
                return TaskComplexity.EXTENSIVE
            elif any(kw in objective_lower for kw in ['compare', 'analyze', 'investigate', 'multiple']):
                return TaskComplexity.COMPLEX
            elif any(kw in objective_lower for kw in ['find', 'what is', 'definition', 'explain']):
                return TaskComplexity.SIMPLE
            else:
                return TaskComplexity.MODERATE
                
        elif task_type == TaskType.ACTION:
            if any(kw in objective_lower for kw in ['implement', 'create', 'build', 'develop']):
                return TaskComplexity.COMPLEX
            elif any(kw in objective_lower for kw in ['update', 'modify', 'fix', 'change']):
                return TaskComplexity.MODERATE
            else:
                return TaskComplexity.SIMPLE
            ...
            research_count = sum(1 for word in research_words if word in objective_lower)
    ...
    def get_effective_tools(self, task_type: TaskType, threshold: float = 0.7) -> List[str]:
        """Get list of effective tools for a task type"""
        effective_tools = []
        
        if task_type in self.patterns['tool_effectiveness']:
            for tool, effectiveness in self.patterns['tool_effectiveness'][task_type].items():
                if effectiveness >= threshold:
                    effective_tools.append(tool)
        
        return effective_tools
    
    def estimate_resources(self, task_type: TaskType, complexity: TaskComplexity) -> Dict[str, float]:
        ...
        if not timing_data:
            ...
            return {
                ...
                'estimated_cost': base_times.get(complexity, 900) / 300,  # Rough cost estimate
                'estimated_iterations': 5 + (5 * list(TaskComplexity).index(complexity)),
                'confidence': 0.0,
            }
        ...
        if successful_runs:
            ...
            avg_iterations = sum(d['iterations'] for d in successful_runs) / len(successful_runs)
            
            return {
                'estimated_time': avg_time * 1.2,  # Add 20% buffer
                'estimated_cost': avg_cost * 1.2,
                'estimated_iterations': int(avg_iterations * 1.2),
                'confidence': min(len(successful_runs) / 10.0, 1.0),  # Confidence based on sample size
            }
        else:
            ...
            avg_iterations = sum(d['iterations'] for d in timing_data) / len(timing_data)
            
            return {
                'estimated_time': avg_time * 1.5,  # Add 50% buffer for unsuccessful patterns
