...
class ReadableTestRunner:
    ...
    def __init__(self, console: Optional[Console] = None, config: Optional[OutputConfig] = None):
    ...
    async def run_test_scenario(self, name: str, description: str, test_func: Callable[[], Awaitable[Dict[str, Any]]]):
            ...
            async def run_with_progress():
                ...
                while not task.done():
                    ...
                    if elapsed > 10 and (elapsed - last_message_time) > 10:
                        self.console.print(f"[dim]🔄 Still processing... ({elapsed:.0f}s elapsed)[/dim]")
                        last_message_time = elapsed
                    elif elapsed > 30 and (elapsed - last_message_time) > 15:
                        self.console.print(f"[dim]⏳ Complex operation in progress... ({elapsed:.0f}s elapsed)[/dim]")
                        last_message_time = elapsed
                    elif elapsed > 60 and (elapsed - last_message_time) > 20:
                        self.console.print(f"[dim]⌛ This is taking longer than usual... ({elapsed:.0f}s elapsed)[/dim]")
                        last_message_time = elapsed
    ...
    def _display_quality_summary(self, metrics: Dict[str, Any]):
        ...
        if metrics.get("premature_attempt"):
            self.console.print("  [yellow]⚠ Test detected premature answer attempt[/yellow]")
        
        verbosity = metrics.get("verbosity", 0)
        if verbosity > 0.7:
            self.console.print(f"  [yellow]⚠ High verbosity detected ({verbosity:.0%})[/yellow]")
    ...
    def _display_bloat_analysis(self, answer_lengths: List[int]):
        ...
        bloat_ratio = answer_lengths[-1] / answer_lengths[0] if answer_lengths[0] > 0 else 1.0
