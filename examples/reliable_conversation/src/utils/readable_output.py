...
class OutputConfig:
    ...
    use_color: bool = True
    max_response_preview: int = 3000  # Very generous - we want to read the conversation!
...
class ReadableFormatter:
    ...
    def format_quality_score(self, score: float, issues: List[str] = None) -> str:
        ...
        result = f"Quality: [{color}]{bar}[/{color}] [{color}]{score:.0%} {icon}[/{color}]"
    ...
    def format_conversation_turn(self, user_input: str, response: str, 
                               quality_metrics: Optional[Dict[str, Any]] = None,
                               turn_number: int = 1) -> None:
    ...
    def _contains_code(self, text: str) -> bool:
        ...
        return "```" in text or bool(re.search(r'\b(def|class|import|function|var|let|const)\b', text))
