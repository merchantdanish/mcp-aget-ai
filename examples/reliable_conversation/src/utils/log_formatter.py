...
def create_readable_summary(message: str, record: logging.LogRecord) -> Optional[str]:
    ...
    if event_type == 'LLM_CALL_START':
        ...
        return f"📊 LLM RESPONSE: received"
    
    elif event_type == 'CONVERSATION_EVENT':
        return f"🔄 CONVERSATION EVENT"
        ...
        return f"⭐ QUALITY EVAL: completed"
    
    elif event_type == 'REQUIREMENTS':
        return f"📋 REQUIREMENTS: extracted"
    
    elif event_type == 'CONTEXT_CONSOLIDATION':
        return f"🔄 CONTEXT: consolidated"
...
class ReadableFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        name = record.name.split('.')[-1] if '.' in record.name else record.name  # Just the last component
