...
class ConversationWorkflow(Workflow[Dict[str, Any]]):
    ...
    async def _run_asyncio_conversation(self, args: Dict[str, Any]) -> WorkflowResult[Dict[str, Any]]:
        ...
        if "state" in args and args["state"]:
            ...
            log_conversation_event(
                self.logger, "conversation_started", 
                self.state.conversation_id
            )
        ...
        response_data = {
            ...
            "metrics": self.state.quality_history[-1].to_dict() if self.state.quality_history else {},
            "turn_number": self.state.current_turn
        }

        log_conversation_event(
            self.logger, "turn_completed", 
            self.state.conversation_id,
            {"turn": self.state.current_turn, "response_length": len(response_data["response"])}
        )
    ...
    async def _add_system_message(self):
        ...
        log_workflow_step(
            self.logger, self.state.conversation_id, 
