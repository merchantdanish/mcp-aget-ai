...
class PropertySearchPreferences(BaseModel):
    ...
    property_types: str = Field(default="all", description="Property types: all, house, condo, townhouse, apartment")
    max_days_on_market: int = Field(default=365, description="Maximum days property has been on market")
    sort_by: str = Field(default="price_low", description="Sort by: price_low, price_high, newest, days_on_market")
    include_rentals: bool = Field(default=True, description="Include rental properties in search?")
...
class MarketAnalysisPreferences(BaseModel):
    ...
    compare_neighborhoods: bool = Field(default=False, description="Compare different neighborhoods?")
    focus_investment: bool = Field(default=False, description="Focus on investment metrics?")

class RentalTrendsPreferences(BaseModel):
    property_size: str = Field(default="all", description="Property size focus: all, studio, 1br, 2br, 3br, 4br+")
    trend_period: str = Field(default="12months", description="Trend analysis period: 6months, 12months, 24months")
    include_vacancy_data: bool = Field(default=True, description="Include vacancy rate data?")
    seasonal_analysis: bool = Field(default=False, description="Include seasonal trend analysis?")


async def make_api_request(endpoint: str, params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    raise Exception(f"RentSpider API error {response.status}: {error_text}")
...
async def get_comparable_properties(property_id: str, ctx: Context) -> str:
    ...
    class CompAnalysisPrefs(BaseModel):
        radius_miles: float = Field(default=1.0, description="Search radius in miles for comparable properties")
        max_comps: int = Field(default=10, description="Maximum number of comparable properties to return")
        include_pending: bool = Field(default=False, description="Include pending sales in comparison?")
    ...
    match result:
        case AcceptedElicitation(data=prefs):
            ...
            try:
                ...
                data = await make_api_request(f"properties/{property_id}/comparables", api_params)
...
def main():
    ...
    if not RENTSPIDER_API_KEY:
        ...
        print("The server will start but API calls will fail until the key is configured.")
