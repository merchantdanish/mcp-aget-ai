...
class ContextAssembler:
    ...
    async def _organize_context(self, company_docs: str, content_samples: str, url_content: str, 
        ...
        try:
            ...
            # UPDATED: Use Agent abstraction instead of direct OpenAIAugmentedLLM
            organizer_agent = Agent(
                name="context_organizer",
                instruction=organizer_prompt,
                server_names=[]
            )
            
            async with organizer_agent:
                organizer_agent.attach_llm(OpenAIAugmentedLLM())
                organized = await organizer_agent.llm.generate_str(
    ...
    async def _get_clarification(self, request: str, platform: str, context: dict) -> dict:
        ...
        try:
            ...
            async with analyzer_agent:
                analyzer_agent.attach_llm(OpenAIAugmentedLLM())
                analysis = await analyzer_agent.llm.generate_str(
