...
# Export to jaeger -- make sure this matches the otlp endpoint in mcp_agent.config.yaml for this example
...
tracer = trace.get_tracer("mcp-sse-server")
...
def main():
    ...
    @server.list_tools()
    @tracer.start_as_current_span("server.list_tools")
    ...
    @server.call_tool()
    @tracer.start_as_current_span("server.call_tool")
