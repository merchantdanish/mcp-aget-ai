...
async def main():
    ...
    async with MCPApp("config/mcp_agent.config.yaml") as app:
        ...
        if workflow._current_memory:
            ...
            print(f"- Total tokens: {memory.total_input_tokens + memory.total_output_tokens:,}")
        ...
        class SecurityAudit(BaseModel):
            vulnerabilities: List[str] = Field(description="List of identified vulnerabilities")
            severity_levels: List[str] = Field(description="Severity for each vulnerability")
        ...
        print(f"[green]Structured Result:[/green]\n{audit_result.model_dump_json(indent=2)}\n")
